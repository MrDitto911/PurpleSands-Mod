...
public class SandfleaEntity extends Animal {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.AMBIENT).add(new MobSpawnSettings.SpawnerData(PurpleSandsModEntities.SANDFLEA, 20, 4, 6));
	}

	public SandfleaEntity(FMLPlayMessages.SpawnEntity packet, Level world) {
		this(PurpleSandsModEntities.SANDFLEA, world);
	}

	public SandfleaEntity(EntityType<SandfleaEntity> type, Level world) {
		super(type, world);
		xpReward = 3;
		setNoAi(false);
	}
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(PurpleSandsModItems.RAW_SANDFLEA));
	}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		SandfleaEntity retval = PurpleSandsModEntities.SANDFLEA.create(serverWorld);
	...
	public boolean isFood(ItemStack stack) {
		return List.of(Items.ROTTEN_FLESH).contains(stack);
	}

	public static void init() {
		SpawnPlacements.register(PurpleSandsModEntities.SANDFLEA, SpawnPlacements.Type.NO_RESTRICTIONS, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
